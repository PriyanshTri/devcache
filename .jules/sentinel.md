...
**Vulnerability:** The application used `fetch()` directly on database-supplied URLs (`item.fileUrl!`) without validation against expected storage domains, allowing Server-Side Request Forgery (SSRF) where attackers could exploit the server to fetch internal resources (e.g. `169.254.169.254`).
**Learning:** Even if data comes from the database, it should be treated as untrusted, especially if user-controllable (e.g. users might insert malicious URLs). When fetching data server-side, ALWAYS validate the URL against an allowed list of domains/prefixes.
**Prevention:** Implement strict prefix checks (e.g., using `process.env.R2_PUBLIC_URL`) on URLs before executing `fetch()`. Parse the URL using `new URL()` to correctly resolve paths, and ensure the configuration exists (fail closed).
