...
## 2024-05-18 - [CRITICAL] Prevent SSRF in file exports
**Vulnerability:** The `/api/export` endpoint fetched file URLs from the database (`item.fileUrl`) without validating them against the expected public storage URL, creating a Server-Side Request Forgery (SSRF) vulnerability.
**Learning:** Even if data is stored in your database, if it represents a URL that your server will fetch, it must be validated. An attacker who managed to manipulate their item's `fileUrl` could force the server to make requests to internal services or malicious endpoints.
**Prevention:** Always validate external URLs against an expected prefix (e.g., `process.env.R2_PUBLIC_URL`) before executing server-side fetches. Ensure the check fails closed if configuration is missing, and explicitly append a trailing slash to the prefix to prevent domain extension bypasses.
