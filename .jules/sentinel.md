...
## 2024-05-18 - [CRITICAL] Prevent SSRF in Data Export Route
**Vulnerability:** The `/api/export` route fetched URLs specified in `item.fileUrl` without validating them against an expected internal prefix.
**Learning:** Even when reading from the database, user-controlled data can result in SSRF if it's blindly passed to server-side `fetch()`.
**Prevention:** Validate external URLs against expected prefixes (e.g., `process.env.R2_PUBLIC_URL`) before executing server-side fetches. Ensure the check 'fails closed' if the configuration is missing, and explicitly append a trailing slash to the prefix before checking `.startsWith()` to prevent domain extension bypasses.
