...
## 2024-05-18 - [CRITICAL] Prevent SSRF in data exports
**Vulnerability:** The `/api/export` endpoint fetched arbitrary URLs defined by `item.fileUrl` without validating that they belong to the trusted `R2_PUBLIC_URL` domain.
**Learning:** Data fetched from the database, even if originally supplied by the system, should still be validated at the time of use if it's passed to sensitive functions like `fetch()`. Otherwise, an attacker modifying the database could trigger Server-Side Request Forgery (SSRF) and extract internal data or perform unauthorized requests, leaking the response via the data export feature.
**Prevention:** Always validate external URLs against an expected prefix (e.g., `process.env.R2_PUBLIC_URL`). Fail closed if the configuration is missing, use `new URL()` to normalize the input (resolving path traversals natively), and explicitly append a trailing slash to the expected prefix before calling `.startsWith()` to prevent domain bypasses (e.g., `https://r2.mybucket.com.attacker.com`).
