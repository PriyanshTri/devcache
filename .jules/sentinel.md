...
**Vulnerability:** The data export endpoint fetched `fileUrl` directly from the database without validation. Since users could hypothetically manipulate their data to point to internal URLs or metadata services, this constituted a Server-Side Request Forgery (SSRF) vulnerability.
**Learning:** When fetching URLs stored in the database, even if originally generated by the application, always re-validate them at the time of fetch.
**Prevention:** Always parse the URL using `new URL()` to resolve relative paths like `..` and ensure it starts with the trusted, expected prefix (with a trailing slash) before executing `fetch()`. Fail closed if the expected configuration is missing.
