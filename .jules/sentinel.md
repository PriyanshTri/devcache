...
## 2024-05-18 - [HIGH] Prevent SSRF in Data Export
**Vulnerability:** The `/api/export` endpoint fetched file URLs directly from the database to include in the ZIP archive without validating that the URL actually belonged to the expected storage domain.
**Learning:** Even if data comes from the database, it should be treated as untrusted, especially when passed to functions like `fetch`. An attacker could inject arbitrary internal or external URLs into their items, causing the server to fetch them (Server-Side Request Forgery).
**Prevention:** Always validate external URLs against an expected prefix (e.g., `process.env.R2_PUBLIC_URL`) before executing server-side fetches. Ensure the check 'fails closed' if the configuration is missing, use `new URL()` to correctly parse and resolve path traversals, and enforce trailing slashes on prefixes to prevent domain bypasses.
