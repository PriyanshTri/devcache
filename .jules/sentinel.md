...
## 2024-05-18 - [CRITICAL] Prevent SSRF in Server-Side Fetches
**Vulnerability:** The application was fetching URLs directly from user-controlled database fields (`item.fileUrl`) without validation in `src/app/api/export/route.ts`, creating a Server-Side Request Forgery (SSRF) vulnerability.
**Learning:** Checking string prefixes for URL validation is insufficient. Attackers can bypass naive `.startsWith('https://expected-domain.com')` checks using domain prefix confusion (e.g., `https://expected-domain.com.attacker.com`).
**Prevention:** Always validate external URLs against expected prefixes by parsing them with `new URL()` first to resolve path traversals natively. Explicitly append a trailing slash to the expected domain prefix before checking `.startsWith()` on the parsed URL's `.href` to prevent domain bypasses. Ensure the check "fails closed" if the configuration is missing.
