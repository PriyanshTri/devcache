...
**Vulnerability:** The data export feature allowed downloading files directly using `fetch(item.fileUrl)`. Since users can set `item.fileUrl` to arbitrary values, this enabled Server-Side Request Forgery (SSRF). An attacker could download files from internal servers or malicious external servers.
**Learning:** External or arbitrary user-provided URLs must be strictly validated before server-side fetching. Relying on simple database associations without structural URL validation is unsafe.
**Prevention:** Always validate external URLs against an expected prefix (e.g., `process.env.R2_PUBLIC_URL`). Parse the URL using `new URL()` to natively resolve path traversals, ensure the configuration has a trailing slash to prevent domain bypasses (`.attacker.com`), and fail closed if the configuration is missing.
