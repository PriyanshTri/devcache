...
## 2024-06-21 - [CRITICAL] Prevent SSRF in Server-Side Fetching
**Vulnerability:** The application was fetching files from user-controlled URLs in the export route (`item.fileUrl`) without validating that the URL belonged to the expected storage bucket.
**Learning:** Fetching unchecked URLs on the server can lead to Server-Side Request Forgery (SSRF), allowing attackers to scan internal networks or access internal cloud metadata services.
**Prevention:** Always validate external URLs against an expected prefix (e.g., `process.env.R2_PUBLIC_URL`) before executing server-side fetches. Ensure the check fails closed, parse the URL using `new URL()`, and explicitly append a trailing slash to the expected prefix before checking `.startsWith()` on the parsed URL's `href` to prevent domain bypasses.
