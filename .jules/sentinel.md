...
## 2024-05-18 - [CRITICAL] Prevent Server-Side Request Forgery (SSRF)
**Vulnerability:** The `/api/export` endpoint fetched files using user-provided URLs (`item.fileUrl`) without validating them, allowing an attacker to coerce the server into making arbitrary HTTP requests (SSRF).
**Learning:** Server-side fetch calls using URLs derived from user data must be strictly validated.
**Prevention:** Validate external URLs against expected prefixes (e.g., `process.env.R2_PUBLIC_URL`) before executing server-side fetches. Use `new URL()` to normalize the path and append a trailing slash to the prefix to prevent domain bypasses. Ensure it fails closed if the configuration is missing.
