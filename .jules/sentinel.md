...
## 2024-07-02 - [CRITICAL] Prevent SSRF in Data Export
**Vulnerability:** The application blindly fetched user-controlled `fileUrl` strings during ZIP export, exposing it to Server-Side Request Forgery (SSRF) where attackers could request internal cloud endpoints or third-party APIs from the server.
**Learning:** Even if a URL field is intended to just store public bucket links, if it can be modified by users (e.g., via import functionality) and later requested by the server, it creates an SSRF vector.
**Prevention:** Always parse the URL natively using `new URL()` to resolve path traversal, and explicitly validate that it starts with the trusted expected prefix (with a trailing slash) before executing server-side fetches.
