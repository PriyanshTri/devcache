...
export function SignInForm() {
  ...
  async function handleCredentialsSubmit(e: React.FormEvent) {
    ...
    if (result?.error) {
      if (result.code === "RateLimitExceeded" || result.error.includes("RateLimitExceeded")) {
        setFormError("Too many login attempts. Please try again later.");
        setNeedsVerification(false);
      } else if (result.code === "credentials" && result.error.includes("EmailNotVerified")) {
  ...
  return (
    <Card className="w-full max-w-md">
      <CardHeader className="text-center">
        <CardTitle className="text-2xl">Sign in to DevCache</CardTitle>
        <CardDescription>
