...
## 2025-05-15 - SSRF Vulnerability in File Export
**Vulnerability:** The ZIP file export API (`src/app/api/export/route.ts`) fetched files directly from URLs provided in the database (`item.fileUrl`) without validating them against an expected host or prefix. An attacker could potentially modify their data to include arbitrary URLs, tricking the server into making requests to internal or unintended external resources.
**Learning:** Even when reading URLs from an internal database, if the data can be influenced by users or represents external resources, it must be validated before being used in a server-side `fetch()`. The database contents should not be implicitly trusted for outgoing requests.
**Prevention:** Always validate external URLs against an expected prefix (e.g., `process.env.R2_PUBLIC_URL`) before executing server-side fetches. Ensure the check fails closed if the configuration is missing, and explicitly append a trailing slash to the prefix before checking `.startsWith()` to prevent domain extension bypasses (e.g., `.attacker.com`).
