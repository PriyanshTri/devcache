...
## 2024-05-24 - Prevent SSRF in export route
**Vulnerability:** Server-Side Request Forgery (SSRF) in export route due to fetching user-supplied `fileUrl`.
**Learning:** The export route fetched `fileUrl` directly from the database without validating the domain against the expected R2 public URL prefix, allowing potential SSRF.
**Prevention:** Always validate external URLs against expected prefixes (e.g., `process.env.R2_PUBLIC_URL + '/'`) before executing server-side fetches.
