...
## 2024-06-02 - [CRITICAL] Prevent SSRF in Data Exports
**Vulnerability:** The /api/export endpoint fetched item.fileUrl directly without validation, potentially allowing SSRF attacks where a malicious user could craft a file URL pointing to internal network resources.
**Learning:** Never trust URLs stored in the database if they are to be fetched server-side, as they could have been manipulated or populated via external APIs.
**Prevention:** Always validate external URLs against an expected configuration prefix (like R2_PUBLIC_URL + '/') before executing server-side fetches.
