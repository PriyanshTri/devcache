...
## 2024-05-10 - Server-Side Request Forgery (SSRF) in Export API
**Vulnerability:** The export API (`src/app/api/export/route.ts`) fetches files directly using `item.fileUrl` without validating that the URL points to the expected R2 bucket. An attacker could potentially store a malicious `fileUrl` pointing to an internal service or a malicious endpoint, causing the server to fetch it during a ZIP export.
**Learning:** External URLs stored in the database must be validated against expected prefixes (like `process.env.R2_PUBLIC_URL`) before executing server-side fetches.
**Prevention:** Always validate that URLs used in server-side `fetch` calls start with expected and safe origins. Ensure configuration check "fails closed" if the configuration is missing, and explicitly append a trailing slash to the prefix before checking `.startsWith()` to prevent domain extension bypasses.
