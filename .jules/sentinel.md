...
## 2024-05-20 - [CRITICAL] Prevent Server-Side Request Forgery (SSRF) in File Export
**Vulnerability:** The `/api/export` route fetched user-provided file URLs (`item.fileUrl`) directly from the database to include them in the generated ZIP export archive without verifying their origin. If an attacker managed to store a malicious URL (e.g., `http://internal-server.local/admin`), the backend would fetch it on their behalf, potentially exposing internal network resources or bypassing firewalls.
**Learning:** Never trust URLs retrieved from the database or provided by the user for server-side fetching, even if they represent application entities like files. The backend must always strictly validate that external requests are only made to expected, safe destinations.
**Prevention:** Before executing server-side fetches based on stored URLs, always validate them against a known safe prefix (e.g., `process.env.R2_PUBLIC_URL`). Ensure the validation includes a trailing slash (e.g., `.startsWith(publicUrl + '/')`) to prevent domain suffix bypasses (like `attacker-publicurl.com`). If the configuration is missing, fail securely by skipping the fetch.
