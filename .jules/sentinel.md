...
## 2024-05-13 - Prevent SSRF in R2 File Export
**Vulnerability:** A Server-Side Request Forgery (SSRF) vulnerability existed in `src/app/api/export/route.ts` where the server fetched user-provided `item.fileUrl` directly without validation while building a ZIP export. An attacker could potentially store an internal or malicious URL in their item's `fileUrl` and have the server fetch it.
**Learning:** Even when fetching data that supposedly originates from a trusted external storage provider (like R2), the actual URL value stored in the database must be validated against expected patterns (like the bucket's public URL) to prevent spoofing and SSRF.
**Prevention:** Always validate external URLs against expected prefixes (e.g., `process.env.R2_PUBLIC_URL`) before executing server-side fetches. Ensure the check "fails closed" (e.g., skipping the fetch if the configuration is missing or the prefix doesn't match), and explicitly append a trailing slash to the prefix before checking `.startsWith()` to prevent domain extension bypasses (e.g., `.attacker.com`).
