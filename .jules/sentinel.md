...
**Vulnerability:** The data export route `src/app/api/export/route.ts` fetched files from user-provided URLs (`item.fileUrl`) without validating that the URL originated from the trusted R2 storage domain.
**Learning:** Any server-side `fetch()` that uses a URL derived from the database or user input without validation creates a Server-Side Request Forgery (SSRF) vulnerability. An attacker could potentially store an item with a malicious internal URL (e.g., `http://169.254.169.254/latest/meta-data/`) and trigger an export to exfiltrate sensitive cloud metadata or internal network resources.
