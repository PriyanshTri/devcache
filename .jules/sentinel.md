...
## 2024-05-18 - [CRITICAL] Prevent SSRF in Export ZIP Generation
**Vulnerability:** The `/api/export` route allowed exporting a ZIP archive containing files stored in R2. It iterated over a user's items and blindly called `fetch(item.fileUrl!)` to retrieve the files. Because the database fields were not strictly validated at fetch time against the trusted R2 endpoint, an attacker who could modify `fileUrl` could force the server to make requests to internal services or arbitrary external domains (Server-Side Request Forgery).
**Learning:** Even if data is retrieved from a database (which implies previous validation), it should not be implicitly trusted for outgoing network requests. Missing validation at the actual point of execution creates an SSRF risk.
**Prevention:** Always validate external URLs immediately before executing server-side fetches. Compare the URL against a trusted expected prefix (e.g., `process.env.R2_PUBLIC_URL`). Ensure the validation 'fails closed' if the prefix is unconfigured, and append a trailing slash to the prefix before checking `.startsWith()` to prevent domain extension bypasses (e.g., preventing `https://bucket.com.attacker.com`).
