...
## 2024-05-14 - Prevent SSRF in ZIP Export Feature
**Vulnerability:** The `/api/export` route iterated through the user's files and fetched external URLs (`fetch(item.fileUrl)`) to append them to the exported ZIP. The user-provided `item.fileUrl` was not validated to ensure it belonged to the expected external origin, allowing an attacker to manipulate `fileUrl` and cause the server to fetch unauthorized internal or local URLs.
**Learning:** Even when the primary use case involves data generated by the system, user-modifiable data (such as item metadata representing external endpoints or file URLs) must always be validated server-side. Fetching arbitrary URLs from user-provided data directly creates Server-Side Request Forgery (SSRF) vulnerabilities.
