...
## 2024-05-18 - [CRITICAL] Prevent SSRF in User File Exports
**Vulnerability:** The `/api/export` endpoint fetched files using `fetch(item.fileUrl!)` without validating that the `fileUrl` belonged to the expected storage domain. An attacker could potentially store a malicious URL in their item data, causing the server to fetch from internal network resources or attacker-controlled domains during export (Server-Side Request Forgery).
**Learning:** Even if data (like `fileUrl`) is originally written by the system during upload, it should be treated as untrusted when read back from the database and used in sensitive server-side operations like `fetch()`, especially if there's any path for users to modify it (e.g., via API manipulation or future features).
**Prevention:** Always validate external URLs against an expected trusted prefix (e.g., `process.env.R2_PUBLIC_URL`) before performing server-side fetches. Parse the URL using `new URL()` to natively resolve path traversals, and strictly enforce the trusted prefix on the parsed URL's `href`.
