...
## 2024-05-18 - [CRITICAL] Prevent SSRF in User Data Exports
**Vulnerability:** The data export API endpoint (`/api/export`) blindly fetched URLs stored in user-controlled item `fileUrl` fields when aggregating attachments. This created a Server-Side Request Forgery (SSRF) vulnerability.
**Learning:** Even fields that are generally populated by the system upon file upload must not be inherently trusted if there is a vector (like an API, direct DB access, or import feature) where they can be maliciously modified by a user to point to internal IP ranges, metadata endpoints, or other external servers.
**Prevention:** Always parse and explicitly validate URLs against a strict allowed prefix list (e.g., the configured `R2_PUBLIC_URL`) using URL parsing before passing them to server-side `fetch()` functions. Ensure the mechanism fails securely (closed) when configurations are missing or URLs cannot be parsed.
