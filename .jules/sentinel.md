...
## 2024-05-18 - [CRITICAL] Prevent SSRF in ZIP Export Feature
**Vulnerability:** The ZIP export feature at `/api/export/route.ts` used `fetch(item.fileUrl!)` without any validation. An attacker could modify an item's `fileUrl` in the database (e.g., via a secondary vulnerability or direct DB access) to point to an internal server or external malicious endpoint, leading to Server-Side Request Forgery (SSRF) when the export is triggered.
**Learning:** Even if data (like `fileUrl`) is assumed to be internally generated (e.g., from an upload endpoint), it must not be implicitly trusted when passed to sensitive functions like `fetch()` if it can be modified.
**Prevention:** Always validate URLs against expected safe prefixes (e.g., `process.env.R2_PUBLIC_URL`) before fetching them server-side. Ensure the prefix is strict (e.g., ends with a `/`) to prevent domain extension bypasses, and fail closed if the configuration is missing or the prefix doesn't match.
