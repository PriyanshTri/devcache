...
## 2024-05-18 - [HIGH] Prevent SSRF in User Export Route
**Vulnerability:** The `/api/export` endpoint fetched files directly from `item.fileUrl` without validating the domain. Malicious users could alter their database records to make the server perform Server-Side Request Forgery (SSRF) against internal services or metadata endpoints.
**Learning:** Even if data originally comes from a trusted process (like a file upload that generates an R2 URL), any URL stored in the database and subsequently requested by the server via `fetch()` must be strictly validated at the time of the request to prevent SSRF, in case the database value was tampered with or bypassed.
**Prevention:** Always parse untrusted or stored URLs using `new URL()` and explicitly validate that the `href` starts with the expected, securely configured domain prefix (including a trailing slash to prevent subdomain bypasses). If the prefix configuration is missing, fail closed.
