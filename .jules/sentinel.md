...
## 2025-01-20 - [CRITICAL] Prevent SSRF in User Data Export
**Vulnerability:** The user data export route (`/api/export`) blindly called `fetch(item.fileUrl)` without verifying the URL's origin. Although `item.fileUrl` is generated on the server, if the database was poisoned or an attacker bypassed upload restrictions to insert a custom URL, they could trick the server into making HTTP requests to internal networks or malicious endpoints.
**Learning:** Server-Side Request Forgery (SSRF) vulnerabilities can occur when a server fetches resources based on database values, not just direct user input. You must explicitly validate these URLs before initiating outbound requests.
**Prevention:** When fetching files or resources server-side, always validate the target URL against an explicit allowlist or a trusted prefix (like `process.env.R2_PUBLIC_URL`), failing securely if configuration is missing.
